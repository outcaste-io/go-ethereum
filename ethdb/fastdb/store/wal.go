...
func (l *wal) allEntries(lo, hi, maxSize uint64) []Entry {
	var entries []Entry
...
func (l *wal) AddEntries(entries []Entry) error {
	if len(entries) == 0 {
		return nil
	}
	...
	if eidx >= 0 {
		if fidx == -1 {
			...
			if l.nextEntryIdx > eidx {
				z.ZeroOut(l.current.Data, entryHeaderSize*eidx, entryHeaderSize*l.nextEntryIdx)
			}
		} else {
			...
			for _, ef := range extra {
				glog.Infof("Deleting extra file: %d\n", ef.fid)
				return fmt.Errorf("No deletions allowed")
			}
			z.ZeroOut(l.current.Data, entryHeaderSize*eidx, logFileOffset)
	...
	for _, re := range entries {
		...
		buf := l.current.getHeader(l.nextEntryIdx)
		marshalHeader(buf, re, uint64(offset))
...
func (l *wal) firstIndex() uint64 {
	...
	var fi uint64
	if len(l.files) == 0 {
		fi = l.current.getHeader(0).Index()
	} else {
		fi = l.files[0].getHeader(0).Index()
	}
...
func (l *wal) LastIndex() uint64 {
	if l.nextEntryIdx-1 >= 0 {
		e := l.current.getHeader(l.nextEntryIdx - 1)
		return e.Index()
	}
...
// getEntryFile returns right logFile corresponding to the fid A value of -1
...
// entry with an index greater than equals to the provided raftInde A
...
func (l *wal) slotGe(raftIndex uint64) (int, int) {
	...
	// fileIdx points to the first log file, whose firstIndex is >= raftInde
	// If the firstIndex == raftIndex, then return.
	if fileIdx < len(l.files) && l.files[fileIdx].firstIndex() == raftIndex {
		return fileIdx, 0
	}
	// Otherwise, go back one file to the file which has firstIndex < raftInde
	if fileIdx > 0 {
		fileIdx--
	}
...
func (l *wal) seekEntry(raftIndex uint64) (entryHeader, error) {
	if raftIndex == 0 {
	...
	if off == -1 {
		...
		// The log has not advanced past the given raftInde
		return emptyEntry, raft.ErrUnavailable
	}

	ef := l.getEntryFile(fidx)
	ent := ef.getHeader(off)
	if ent.Index() == 0 {
		// The log has not advanced past the given raftInde
		return emptyEntry, raft.ErrUnavailable
	}
...
// Term returns the term of entry with raft index = id It returns an error if
...
func openWal(dir string) (*wal, error) {
	...
	for _, ef := range files {
		...
		if ef.firstIndex() == 0 {
			return nil, fmt.Errorf("deletion prohibited -- first index is zero for %s", ef.Fd.Name())
		} else {
			out = append(out, ef)
		}
	}
